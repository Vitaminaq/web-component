...
export default defineComponent({
    ...
    getAttributesToRender() {
      if (this.renderer.element.attributes) {
        return Object.fromEntries(
          this.renderer.element.attributes.map((attribute: any) => [
            attribute.name,
            attribute.value
          ])
        )
  ...
  async serverPrefetch() {
    ...
    try {
      ...
      console.log(this.litSsrHtml, '8888888888888888888')
    } catch (e) {
      console.error(`[web component]：${this.litElementTagName}服务端渲染失败`, e)
      this.litSsrHtml = ''
    }
  },
